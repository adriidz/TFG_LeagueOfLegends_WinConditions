...
class Config:
    raw_root: str = os.getenv("RAW_ROOT", "Data/raw")
    routing_region: str = os.getenv("REGION", "europe")
    out_csv: str = os.getenv("LABELS_OUT", "Data/dataset_v1.csv")
    ...
    # Parche major.minor para análisis (ej: 13.12)
...
def patch_major_minor(game_version: Optional[str]) -> Optional[str]:
    if not game_version:
        return None
    sp = str(game_version).split(".")
    return ".".join(sp[:2]) if len(sp) >= 2 else str(game_version)
...
def obs_team_focus_early(frames, team_id, participants, cfg):
    ...
    for i in range(1, limit):
        ...
        side = get_map_side_simple(pos.get("x"), pos.get("y"))
        if side == "TOP":
            top_frames += 1
        elif side == "BOT":
            bot_frames += 1
...
def obs_map_mode_mid(frames: List[dict], team_id: int, participants: List[dict], cfg: Config) -> str:
    ...
    # En v1: split = no-group sistemático
...
def process(cfg: Config):
    ...
    # Contadores
    # -----------------------------
    raw_dirs_total = 0                 # carpetas encontradas (matchId)
    raw_dirs_valid_json = 0            # carpetas que pudimos leer (match+timeline)
    raw_dirs_pass_patch = 0            # matches >= min patch

    rows_total = 0                     # filas totales generadas (normalmente 2 por match válido)
    rows_pass_patch = 0                # filas generadas solo de matches que pasan patch
    ...
    for md in iter_match_dirs(cfg):
        ...
        if pass_patch:
            ...
            # Si quieres que el CSV SOLO incluya >=16.2, deja este continue.
            continue

        # (tu lógica normal a partir de aquí)
        ...
        # vamos a generar 2 filas (blue/red) para este match
        for team_id in (100, 200):
            ...
            if has_mid:
                ...
                row["SUCCESS_MAP_MODE_MID"] = success_map_mode_mid(mmode, ev_14_25)
            else:
                row["OBS_MAP_MODE_MID"] = None
                row["SUCCESS_MAP_MODE_MID"] = None
    ...
    # escribir CSV (igual que antes)
    base_cols = [
        "matchId","teamId","patch",
        "window_early","window_mid",
        "is_neutral_early","gold_diff_14","tower_diff_14",
        "OBS_TEAM_FOCUS_EARLY","SUCCESS_TEAM_FOCUS_EARLY",
        "OBS_JG_STYLE_EARLY","SUCCESS_JG_STYLE_EARLY",
        "OBS_MAP_MODE_MID","SUCCESS_MAP_MODE_MID",
    ]
    ...
    print("\n=== CONTADORES FILAS GENERADAS ===")
    print(f"Filas generadas totales:                    {rows_total}")
    print(f"Filas generadas (parche >= {cfg.min_patch_major}.{cfg.min_patch_minor}): {rows_pass_patch} ({pct(rows_pass_patch, rows_total):.1f}%)")
    ...
    print(f"¡Éxito! CSV generado con {len(rows)} filas en {cfg.out_csv}")

    # Reporte rápido (como en vuestro v1)
    if rows:
        neutral_rows = [r for r in rows if r.get("is_neutral_early") == 1]
        print("\n--- REPORTE (solo neutrales early) ---")
        print(f"Muestras neutrales: {len(neutral_rows)} / {len(rows)} ({(len(neutral_rows)/len(rows))*100:.1f}%)")

        def dist(key: str):
            c = Counter(r.get(key) for r in neutral_rows)
