...
TOTAL_MATCHES_TARGET = 200000
CSV_FILENAME = "Data/dataset_raw.csv"
SAVE_EVERY_N_MATCHES = 100  # Guardar cada N partidas para no perder todo en caso de error
...
def get_match_ids_safe(puuid, count):
    """Obtiene IDs de partidas paginando (count puede ser >100).

    Riot limita `count` a 100 por request, pero permite paginar con `start`.
    Esta función devuelve como máximo `count` IDs (si el jugador tiene menos, devuelve menos).
    """
    match_ids = []
    start = 0
    remaining = int(count)
    ...
    while remaining > 0:
        batch_count = 100 if remaining > 100 else remaining
        try:
            batch = watcher.match.matchlist_by_puuid(
                REGION,  # NO cambiar: el usuario indica que esto funciona en su entorno
                puuid,
                start=start,
                count=batch_count,
                queue=420
        ...
        # Si la API devuelve menos de lo pedido, normalmente no hay más páginas.
        if got < batch_count:
            break

        # Ser amable con la API (RiotWatcher también maneja 429, pero esto reduce picos)
        time.sleep(0.2)
    ...
    for idx, entry in enumerate(players):
        ...
        for m_id in new_ids:
            ...
            if unsaved_count >= SAVE_EVERY_N_MATCHES and new_data_buffer:
                ...
                unsaved_count = 0
                print(f"  -> Guardado parcial (partida) en CSV {CSV_FILENAME} (Total partidas: {matches_collected})")
        ...
        if new_data_buffer:
            ...
            schema_cols = append_csv_consistent(CSV_FILENAME, df, schema_cols)
            new_data_buffer = []  # Limpiar buffer
            print(f"  -> Guardado parcial (jugador) en CSV {CSV_FILENAME} (Total partidas: {matches_collected})")
