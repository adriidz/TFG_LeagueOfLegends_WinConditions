#!/usr/bin/env python3
"""
04c_train_mlp_per_role.py -- MLP with per-role champion embeddings.

Compared with 04b, this model gives each draft slot its own embedding table.
It also adds two explicit interaction features requested in the roadmap:

  - dot(ally_utility, enemy_utility): support-vs-support matchup
  - dot(ally_utility, ally_bottom): support-ADC synergy

Validation is used for model selection. The final held-out test comparison is
generated by 07_model_comparison.py.
"""
...
DEFAULT_OUTDIR = str(REPO_ROOT / "final" / "models" / "mlp_per_role")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train MLP with per-role champion embeddings.")
