...
def build_window_output_paths(base_suffix: str, max_minute: float, multi_window: bool) -> Dict[str, str]:
    jg_path = apply_window_suffix(os.path.join(DEFAULT_OUT_DIR_LABELS, f"{JUNGLE_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    sp_path = apply_window_suffix(os.path.join(DEFAULT_OUT_DIR_LABELS, f"{SUPPORT_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    tm_path = apply_window_suffix(os.path.join(DEFAULT_OUT_DIR_LABELS, f"{TEAM_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    return {
        "jungle": jg_path,
        "support": sp_path,
        "team": tm_path,
...
def main() -> None:
    args = parse_args()
    raw_base = os.path.join(DEFAULT_RAW_ROOT, DEFAULT_REGION)
    ...
    # Rutas de salida
    dr_path = os.path.join(DEFAULT_OUT_DIR_FEATURES, f"{DRAFT_OUT}{suffix}.parquet")
    ensure_parent_dir(dr_path)
    dr_analysis = os.path.splitext(dr_path)[0] + "_analysis"
    ensure_dir(dr_analysis)

    window_output_paths = {m: build_window_output_paths(suffix, m, multi_window) for m in analysis_windows}
    ...
    if not df_dr.empty:
        ...
        save_dataframe(
            pd.DataFrame([{
                ...
                "windows": ",".join(str(m) for m in analysis_windows),
