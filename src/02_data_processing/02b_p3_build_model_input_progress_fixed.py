#!/usr/bin/env python3
"""
02b_build_model_input_progress.py

Versión parametrizable de 02b_build_model_input.py para experimentos EDA.
Permite indicar rutas explícitas de entrada/salida sin tocar los defaults del pipeline.
"""
...
GLOBAL_LABEL_RUN_METADATA_COLS = {
    "max_minute",
    "window_tag",
    "label_schema",
    "labeling_mode",
    "quantile_or_threshold_tag",
}

TASK_THRESHOLD_METADATA_COLS = {
    "label_lower_threshold",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--draft-path", default=None,
                        help="Ruta base de draft_features.parquet. Si no se pasa, usa el default del pipeline.")
    parser.add_argument("--jungle-labels-path", default=None,
                        help="Ruta base de jungle_labels.parquet.")
    parser.add_argument("--support-labels-path", default=None,
                        help="Ruta base de support_labels.parquet.")
    parser.add_argument("--team-labels-path", default=None,
                        help="Ruta base de team_tendency_labels.parquet.")
    parser.add_argument("--out-path", default=None,
                        help="Ruta base de salida del model_input parquet.")
    parser.add_argument("--summary-dir", default=None,
                        help="Directorio base para tablas de análisis. Se le añaden sufijos automáticamente.")
    parser.add_argument(
        "--join-how",
        choices=["left", "inner"],
        default="left",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Carga labels generadas para una ventana concreta (ej. 8 -> *_m08.parquet).",
    )
    parser.add_argument(
        "--label-schema",
        choices=["ternary", "binary_clean", "binary_full"],
        default="ternary",
        help="Esquema de discretización usado al construir labels.",
    )
...
def select_label_columns(
    task_name: str,
    label_df: pd.DataFrame,
    draft_df: pd.DataFrame,
    model_df: Optional[pd.DataFrame] = None,
) -> Tuple[pd.DataFrame, List[str]]:
    """Selecciona y sanea columnas de labels antes del merge.

    Reglas:
    - conserva JOIN_KEYS;
    - elimina columnas ya presentes en draft_features;
    - elimina metadatos globales compartidos entre tareas para reinyectarlos una sola vez;
    - renombra thresholds por tarea para evitar colisiones;
    - elimina duplicados respecto al model_df acumulado (manteniendo la primera copia).
    """
    model_cols = set(model_df.columns) if model_df is not None else set()

    rename_map: Dict[str, str] = {
        col: f"{task_name}_{col}"
        for col in TASK_THRESHOLD_METADATA_COLS
    ...
    for col in label_df.columns:
        ...
        if col in draft_df.columns:
            ...
            continue
        if col in GLOBAL_LABEL_RUN_METADATA_COLS:
            dropped_cols.append(col)
            continue
        out_col = rename_map.get(col, col)
        if out_col in model_cols and out_col not in JOIN_KEYS:
            dropped_cols.append(col)
            continue
        keep_cols.append(col)

    out = label_df[keep_cols].copy()
    if rename_map:
        out = out.rename(columns=rename_map)
    return out, dropped_cols


def inject_global_label_run_metadata(
    ...
    *,
    ...
    lower_thr: Optional[float],
    upper_thr: Optional[float],
    labeling_mode: Optional[str] = None,
    max_minute: Optional[float] = None,
    window_tag: Optional[str] = None,
) -> pd.DataFrame:
    out = model_df.copy()
    out["label_schema"] = label_schema
    out["quantile_or_threshold_tag"] = format_quantile_or_threshold_tag(
        lower_q, upper_q, lower_thr, upper_thr
    )
    ...
    if window_tag is not None:
        out["window_tag"] = window_tag
    return out


def get_first_non_null(df: pd.DataFrame, column: str):
...
def write_summary_tables(
    ...
    overall_summary = pd.DataFrame([
        {
            ...
            "labeling_mode": model_df["labeling_mode"].dropna().iloc[0] if "labeling_mode" in model_df.columns and model_df["labeling_mode"].notna().any() else None,
    ...
    label_counts_parts = [
        build_label_counts(model_df, JUNGLE_LABEL_COL, "jungle"),
        build_label_counts(model_df, SUPPORT_LABEL_COL, "support"),
        build_label_counts(model_df, TEAM_LABEL_COL, "team"),
    ]
    non_empty = [df for df in label_counts_parts if not df.empty]
    label_counts = pd.concat(non_empty, ignore_index=True) if non_empty else pd.DataFrame()
...
def resolve_base_paths(args: argparse.Namespace) -> Tuple[str, str, str, str, str, Optional[str]]:
    ...
    jungle_labels_path = args.jungle_labels_path or DEFAULT_JUNGLE_LABELS_PATH
    support_labels_path = args.support_labels_path or DEFAULT_SUPPORT_LABELS_PATH
    team_labels_path = args.team_labels_path or DEFAULT_TEAM_LABELS_PATH
    ...
    if target_frac is not None and 0.0 < target_frac < 1.0:
        ...
        jungle_labels_path = apply_sample_suffix(jungle_labels_path, target_frac)
        support_labels_path = apply_sample_suffix(support_labels_path, target_frac)
        team_labels_path = apply_sample_suffix(team_labels_path, target_frac)
    ...
    if args.label_max_minute is not None:
        ...
        out_path = apply_window_suffix(out_path, args.label_max_minute)
        if summary_dir:
            summary_dir = append_suffix_to_dir(summary_dir, format_window_tag(args.label_max_minute))
    ...
    out_path = apply_quantile_suffix(
        out_path, args.lower_quantile, args.upper_quantile, args.lower_threshold, args.upper_threshold
    )
    out_path = apply_schema_suffix(out_path, args.label_schema)
    if summary_dir:
        summary_dir = append_suffix_to_dir(summary_dir, quantile_tag)
        summary_dir = append_suffix_to_dir(summary_dir, args.label_schema)
    print(f"Fase 2: esquema de labels = {args.label_schema}.")
    print(f"Fase 2: tag de quantiles/thresholds = {quantile_tag}.")

    return draft_path, jungle_labels_path, support_labels_path, team_labels_path, out_path, summary_dir


def main() -> None:
    args = parse_args()

    draft_path, jungle_labels_path, support_labels_path, team_labels_path, out_path_str, summary_dir = resolve_base_paths(args)
    ...
    global_run_metadata: Dict[str, Optional[object]] = {
        "labeling_mode": None,
        "max_minute": None,
        "window_tag": None,
    }
    ...
    for task_name, path, required_cols in task_specs:
        ...
        if global_run_metadata["labeling_mode"] is None:
            global_run_metadata["labeling_mode"] = get_first_non_null(task_df, "labeling_mode")
        if global_run_metadata["max_minute"] is None:
            global_run_metadata["max_minute"] = get_first_non_null(task_df, "max_minute")
        if global_run_metadata["window_tag"] is None:
            global_run_metadata["window_tag"] = get_first_non_null(task_df, "window_tag")
        task_df, dropped_cols = select_label_columns(task_name, task_df, draft_df, model_df)
        if dropped_cols:
            print(f"  - {task_name}: columnas omitidas para evitar merge sucio: {sorted(set(dropped_cols))}")
    ...
    if not loaded_task_tables:
        raise SystemExit(
            "No se ha cargado ninguna tabla de labels. El model_input quedaría sin targets. Revisa las rutas de labels, la ventana y el esquema."
        )

    resolved_max_minute = float(args.label_max_minute) if args.label_max_minute is not None else global_run_metadata["max_minute"]
    resolved_window_tag = format_window_tag(args.label_max_minute) if args.label_max_minute is not None else global_run_metadata["window_tag"]
    model_df = inject_global_label_run_metadata(
        model_df,
        label_schema=args.label_schema,
        lower_q=args.lower_quantile,
        upper_q=args.upper_quantile,
        lower_thr=args.lower_threshold,
        upper_thr=args.upper_threshold,
        labeling_mode=global_run_metadata["labeling_mode"],
        max_minute=resolved_max_minute,
        window_tag=resolved_window_tag,
    )

    selected_tasks = [t for t in (args.keep_only_tasks or list(loaded_task_tables.keys())) if t in loaded_task_tables]
    missing_selected = [t for t in (args.keep_only_tasks or []) if t not in loaded_task_tables]
    if missing_selected:
        raise SystemExit(
