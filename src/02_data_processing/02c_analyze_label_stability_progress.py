#!/usr/bin/env python3
"""
02c_analyze_label_stability_progress.py

Versión compatible con los nombres de archivo actuales del pipeline, incluyendo:
- sufijo de sample: _sampleXX
- sufijo de ventana: _m06, _m08, ...
- sufijo de quantiles/thresholds: _q20_80 o _thrX_Y
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--lower-quantile", type=float, default=0.20,
                   help="Solo para resolver el sufijo de archivo cuando se usan quantiles.")
    p.add_argument("--upper-quantile", type=float, default=0.80,
                   help="Solo para resolver el sufijo de archivo cuando se usan quantiles.")
    p.add_argument("--lower-threshold", type=float, default=None,
                   help="Solo para resolver el sufijo de archivo cuando se usan thresholds fijos.")
    p.add_argument("--upper-threshold", type=float, default=None,
                   help="Solo para resolver el sufijo de archivo cuando se usan thresholds fijos.")
    p.add_argument("--disable-quantile-suffix", action="store_true",
                   help="Usa esta opción solo si tus labels NO llevan sufijo _qXX_YY / _thrX_Y.")
    return p.parse_args()
...
def build_input_path(
    ...
    *,
    ...
    lower_thr: Optional[float],
    upper_thr: Optional[float],
    disable_quantile_suffix: bool,
) -> str:
    sfx = sample_suffix(sample_frac)
    tag = window_tag(window)
    filename = f"{prefix}{sfx}_{tag}"
    if not disable_quantile_suffix:
        filename += "_" + format_quantile_or_threshold_tag(lower_q, upper_q, lower_thr, upper_thr)
    filename += ".parquet"
...
def load_task_window_df(
    ...
    *,
    ...
    path = build_input_path(
        ...
        lower_thr=lower_thr,
        upper_thr=upper_thr,
        disable_quantile_suffix=disable_quantile_suffix,
    )
...
def main() -> None:
    ...
    for task_name in TASK_SPECS:
        ...
        aligned = build_aligned_table(
            ...
            disable_quantile_suffix=args.disable_quantile_suffix,
        )
    ...
    run_summary = pd.DataFrame([{
        ...
        "quantile_or_threshold_tag": None if args.disable_quantile_suffix else format_quantile_or_threshold_tag(
