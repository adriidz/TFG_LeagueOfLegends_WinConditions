#!/usr/bin/env python3
"""
new_02a_build_labels_and_draft_features.py
...
y produce en paralelo:
  ...
  - tablas de scores continuos por tarea y ventana (jungle/support/team)
...
Además exporta tablas de scores continuos para la rama nueva de regresión.
...
DEFAULT_OUT_DIR_LABELS = os.path.join("data_new", "clean", "labels")
DEFAULT_OUT_DIR_FEATURES = os.path.join("data_new", "clean", "features")
DEFAULT_OUT_DIR_SCORES = os.path.join("data_new", "clean", "scores")

JUNGLE_OUT = "jungle_labels"
SUPPORT_OUT = "support_labels"
TEAM_OUT = "team_tendency_labels"
DRAFT_OUT = "draft_features"
JUNGLE_SCORE_OUT = "jungle_scores"
SUPPORT_SCORE_OUT = "support_scores"
TEAM_SCORE_OUT = "team_tendency_scores"
...
def build_window_score_output_paths(out_scores_dir: str, base_suffix: str, max_minute: float, multi_window: bool) -> Dict[str, str]:
    jg_path = apply_window_suffix(os.path.join(out_scores_dir, f"{JUNGLE_SCORE_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    sp_path = apply_window_suffix(os.path.join(out_scores_dir, f"{SUPPORT_SCORE_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    tm_path = apply_window_suffix(os.path.join(out_scores_dir, f"{TEAM_SCORE_OUT}{base_suffix}.parquet"), max_minute, multi_window)
    return {
        "jungle": jg_path,
        "support": sp_path,
        "team": tm_path,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out-scores-dir", default=DEFAULT_OUT_DIR_SCORES,
                   help="Directorio de salida para score tables parquet y análisis asociados.")
    ...
    p.add_argument("--support-score-start-minute", type=float, default=5.0,
                   help="Minuto a partir del cual empieza a agregarse la señal v2 de support.")
    p.add_argument("--support-far-adc-threshold", type=float, default=2500.0,
                   help="Umbral de distancia support-ADC para considerar separación fuerte en v2.")
    p.add_argument("--min-support-frames-v2", type=int, default=4,
                   help="Número mínimo de frames válidos para construir support_roam_score_v2.")
...
def _safe_participant_xp(pf: Optional[dict]) -> Optional[float]:
    if not pf:
        return None
    xp = pf.get("xp")
    if xp is None:
...
def compute_support_metrics_v2(
    frames: List[dict], support_pid: int, adc_pid: int,
    team_id: int, *, start_minute: float = 5.0, far_adc_threshold: float = 2500.0,
    min_support_frames_v2: int = 4, xp_ratio_min: float = 0.60, xp_ratio_max: float = 1.00,
) -> Optional[dict]:
    """Calcula una v2 del target de support basada en separación espacial y gap de XP.

    Mejoras frente a v1:
    - ignora la fase de líneas obligatoria inicial (start_minute)
    - excluye base/fuente del support
    - actualiza XP antes de cualquier continue
    - no depende de un mínimo duro de frames cooperativos
    """
    valid_support_frames_v2 = 0
    valid_coop_frames_v2 = 0
    frames_in_bot_extended = 0
    frames_out_bot_extended = 0
    frames_far_from_adc = 0
    dist_sum = 0.0

    last_support_xp: Optional[float] = None
    last_adc_xp: Optional[float] = None

    total_frames_seen = 0
    post_lane_frames_seen = 0

    roam_episode_count_v2 = 0
    in_roam_episode_prev = False
    ...
    for frame in frames:
        ...
        # XP siempre antes de cualquier filtro
        supp_xp = _safe_participant_xp(supp_pf)
        adc_xp = _safe_participant_xp(adc_pf)
        if supp_xp is not None:
            last_support_xp = supp_xp
        if adc_xp is not None:
            last_adc_xp = adc_xp
        ...
        zone_s = classify_map_zone(pos_s[0], pos_s[1], team_id)
        if zone_s in BASE_ZONES:
            in_roam_episode_prev = False
            continue

        valid_support_frames_v2 += 1

        in_bot_extended = (zone_s in BOT_SIDE_ZONES) or (zone_s == "RIVER_BOT")
        if in_bot_extended:
            frames_in_bot_extended += 1
            in_roam_episode = False
        else:
            frames_out_bot_extended += 1
            in_roam_episode = True

        if in_roam_episode and not in_roam_episode_prev:
            roam_episode_count_v2 += 1
        in_roam_episode_prev = in_roam_episode

        if participant_is_alive(adc_pf):
            pos_a = extract_position(adc_pf)
            if pos_a is not None:
                zone_a = classify_map_zone(pos_a[0], pos_a[1], team_id)
                if zone_a not in BASE_ZONES:
                    dist = math.hypot(pos_s[0] - pos_a[0], pos_s[1] - pos_a[1])
                    dist_sum += dist
                    valid_coop_frames_v2 += 1
                    if dist >= far_adc_threshold:
                        frames_far_from_adc += 1

    if valid_support_frames_v2 < min_support_frames_v2:
        return None

    outside_ratio = frames_out_bot_extended / valid_support_frames_v2
    mean_distance_to_adc_v2 = (dist_sum / valid_coop_frames_v2) if valid_coop_frames_v2 > 0 else None
    far_ratio = (frames_far_from_adc / valid_coop_frames_v2) if valid_coop_frames_v2 > 0 else None

    support_adc_xp_ratio_v2 = None
    if last_support_xp is not None and last_adc_xp is not None and last_adc_xp > 0:
        support_adc_xp_ratio_v2 = last_support_xp / last_adc_xp

    xp_gap = None
    if support_adc_xp_ratio_v2 is not None and xp_ratio_max > xp_ratio_min:
        xp_ratio_clipped = min(max(support_adc_xp_ratio_v2, xp_ratio_min), xp_ratio_max)
        xp_gap = 1.0 - ((xp_ratio_clipped - xp_ratio_min) / (xp_ratio_max - xp_ratio_min))

    post_lane_frame_ratio = (valid_support_frames_v2 / post_lane_frames_seen) if post_lane_frames_seen > 0 else None

    score_num = 0.45 * outside_ratio
    score_den = 0.45
    if far_ratio is not None:
        score_num += 0.35 * far_ratio
        score_den += 0.35
    if xp_gap is not None:
        score_num += 0.20 * xp_gap
        score_den += 0.20
    if score_den <= 0.0:
        return None

    support_roam_score_v2 = score_num / score_den
    coop_ratio_v2 = (valid_coop_frames_v2 / valid_support_frames_v2) if valid_support_frames_v2 > 0 else None
    support_score_confidence_v2 = min(1.0, valid_support_frames_v2 / 6.0)

    return {
        "valid_support_frames_v2": valid_support_frames_v2,
        "valid_coop_frames_v2": valid_coop_frames_v2,
        "coop_frame_ratio_v2": coop_ratio_v2,
        "frames_in_bot_extended": frames_in_bot_extended,
        "frames_out_bot_extended": frames_out_bot_extended,
        "frames_far_from_adc": frames_far_from_adc,
        "roam_episode_count_v2": roam_episode_count_v2,
        "mean_distance_to_adc_v2": mean_distance_to_adc_v2,
        "support_adc_xp_ratio_v2": support_adc_xp_ratio_v2,
        "post_lane_frame_ratio": post_lane_frame_ratio,
        "support_score_confidence_v2": support_score_confidence_v2,
        "support_roam_score_v2": support_roam_score_v2,
        "support_v2_definition": (
            "0.45*out_bot_extended + 0.35*far_from_adc + 0.20*xp_gap; "
            "xp_gap=minmax[0.60,1.00]; start_minute>=5; base_excluded"
        ),
    }
...
def build_score_export_df(df: pd.DataFrame, score_col: str, extra_cols: List[str]) -> pd.DataFrame:
    keep_cols = [
        "match_id", "team_id", "side", "patch", "game_version", "game_start_timestamp",
        "max_minute", "window_tag", score_col,
    ]
...
def main() -> None:
    ...
    score_output_paths = {m: build_window_score_output_paths(args.out_scores_dir, suffix, m, multi_window) for m in analysis_windows}
    ...
    for paths in score_output_paths.values():
        for key in ("jungle", "support", "team"):
            ensure_parent_dir(paths[key])
        for key in ("jungle_analysis", "support_analysis", "team_analysis"):
            ensure_dir(paths[key])
    ...
    print(f"[Rutas] Scores dir: {os.path.abspath(args.out_scores_dir)}")
    ...
    for max_minute in analysis_windows:
        ...
        score_paths = score_output_paths[max_minute]
        ...
        print(f"                       jg_score={os.path.abspath(score_paths['jungle'])}")
        print(f"                       sp_score={os.path.abspath(score_paths['support'])}")
        print(f"                       tm_score={os.path.abspath(score_paths['team'])}")
    print()
    ...
    for mdir in match_dirs:
        ...
        for team_id in (BLUE_TEAM_ID, RED_TEAM_ID):
            ...
            for window_max_minute, frames in frames_by_window.items():
                ...
                # ── Support ──
                sp = compute_support_metrics(
                    frames, sup_pid, adc_pid, team_id,
                    args.min_support_frames, args.min_coop_frames,
                )
                sp_v2 = compute_support_metrics_v2(
                    frames,
                    sup_pid,
                    adc_pid,
                    team_id,
                    start_minute=args.support_score_start_minute,
                    far_adc_threshold=args.support_far_adc_threshold,
                    min_support_frames_v2=args.min_support_frames_v2,
                )
                if sp is not None or sp_v2 is not None:
                    sp_row = dict(sp or {})
                    if sp_v2 is not None:
                        sp_row.update(sp_v2)
                    sp_row.update(common)
                    sp_row["support_participant_id"] = sup_pid
                    sp_row["support_champion_id"] = sup_meta.get("championId")
                    sp_row["support_champion_name"] = sup_meta.get("championName")
                    sp_row["adc_participant_id"] = adc_pid
                    sp_row["adc_champion_id"] = adc_meta.get("championId")
                    sp_row["adc_champion_name"] = adc_meta.get("championName")
                    sp_rows_by_window[window_max_minute].append(sp_row)
                else:
                    sp_reject += 1
    ...
    for window_max_minute in analysis_windows:
        ...
        if not df_jg.empty:
            ...
            jg_score_export = build_score_export_df(
                df_jg,
                "jungle_presence_score",
                [
                    "valid_jungle_frames", "frames_used_for_score", "score_coverage_ratio",
                    "active_map_frames", "own_jungle_frames", "active_map_definition",
                    "jungle_participant_id", "jungle_champion_id", "jungle_champion_name",
                ],
            )
            jg_score_paths = score_output_paths[window_max_minute]
            jg_score_export.to_parquet(jg_score_paths["jungle"], index=False)
            save_dataframe(build_percentiles(jg_score_export["jungle_presence_score"], "jungle_presence_score"),
                           os.path.join(jg_score_paths["jungle_analysis"], "percentiles"))
            save_dataframe(summarize_by_side(jg_score_export, "jungle_presence_score"),
                           os.path.join(jg_score_paths["jungle_analysis"], "side_summary"))
            if not args.skip_plots:
                plot_hist(jg_score_export["jungle_presence_score"],
                          os.path.join(jg_score_paths["jungle_analysis"], "score_dist.png"),
            ...
            if not args.skip_plots:
                plot_hist(df_jg["jungle_presence_score"],
                          ...
                          f"Jungle Presence Score Distribution (0-{window_max_minute:g})", "jungle_presence_score")
            print(f"\n✓ Jungle scores [{window_max_minute:g}]: {len(jg_score_export)} rows → {jg_score_paths['jungle']}")
            print(f"✓ Jungle labels [{window_max_minute:g}]: {len(df_jg)} rows → {paths['jungle']}")
        ...
        if not df_sp.empty:
            ...
            sp_score_export = build_score_export_df(
                df_sp,
                "support_roam_score",
                [
                    "valid_support_frames", "valid_coop_frames", "coop_frame_ratio",
                    "frames_in_botside", "frames_out_botside", "mean_distance_to_adc",
                    "valid_support_frames_v2", "valid_coop_frames_v2", "coop_frame_ratio_v2",
                    "frames_in_bot_extended", "frames_out_bot_extended", "frames_far_from_adc",
                    "roam_episode_count_v2", "mean_distance_to_adc_v2", "support_adc_xp_ratio_v2",
                    "post_lane_frame_ratio", "support_score_confidence_v2", "support_roam_score_v2",
                    "support_v2_definition",
                    "support_participant_id", "support_champion_id", "support_champion_name",
                    "adc_participant_id", "adc_champion_id", "adc_champion_name",
                ],
            )
            sp_score_paths = score_output_paths[window_max_minute]
            sp_score_export.to_parquet(sp_score_paths["support"], index=False)
            save_dataframe(build_percentiles(sp_score_export["support_roam_score"], "support_roam_score"),
                           os.path.join(sp_score_paths["support_analysis"], "percentiles"))
            save_dataframe(summarize_by_side(sp_score_export, "support_roam_score"),
                           os.path.join(sp_score_paths["support_analysis"], "side_summary"))
            if not args.skip_plots:
                plot_hist(sp_score_export["support_roam_score"],
                          os.path.join(sp_score_paths["support_analysis"], "score_dist.png"),
            ...
            if not args.skip_plots:
                plot_hist(df_sp["support_roam_score"],
                          ...
                          f"Support Roam Score Distribution (0-{window_max_minute:g})", "support_roam_score")
            print(f"\n✓ Support scores [{window_max_minute:g}]: {len(sp_score_export)} rows → {sp_score_paths['support']}")
            print(f"✓ Support labels [{window_max_minute:g}]: {len(df_sp)} rows → {paths['support']}")
            print("  Score v1 = ratio_fuera (sin normalización, sin leakage)")
            if "support_roam_score_v2" in df_sp.columns:
                print("  Score v2 añadido en paralelo: separación ADC + XP gap + recorte temporal")
        ...
        if not df_tm.empty:
            ...
            tm_score_export = build_score_export_df(
                df_tm,
                "team_side_focus_score",
                [
                    "valid_frames_any_alive", "frames_with_any_side_signal",
                    "frames_with_top_signal", "frames_with_bot_signal",
                    "top_score", "bot_score", "total_side_mass", "side_signal_coverage_ratio",
                    "role_weights",
                    "jungle_participant_id", "jungle_champion_id", "jungle_champion_name",
                    "support_participant_id", "support_champion_id", "support_champion_name",
                    "mid_participant_id", "mid_champion_id", "mid_champion_name",
                ],
            )
            tm_score_paths = score_output_paths[window_max_minute]
            tm_score_export.to_parquet(tm_score_paths["team"], index=False)
            save_dataframe(build_percentiles(tm_score_export["team_side_focus_score"], "team_side_focus_score"),
                           os.path.join(tm_score_paths["team_analysis"], "percentiles"))
            save_dataframe(summarize_by_side(tm_score_export, "team_side_focus_score"),
                           os.path.join(tm_score_paths["team_analysis"], "side_summary"))
            if not args.skip_plots:
                plot_hist(tm_score_export["team_side_focus_score"],
                          os.path.join(tm_score_paths["team_analysis"], "score_dist.png"),
            ...
            if not args.skip_plots:
                plot_hist(df_tm["team_side_focus_score"],
                          ...
                          f"Team Side Focus Score Distribution (0-{window_max_minute:g})", "team_side_focus_score")
            print(f"\n✓ Team tendency scores [{window_max_minute:g}]: {len(tm_score_export)} rows → {tm_score_paths['team']}")
            print(f"✓ Team tendency labels [{window_max_minute:g}]: {len(df_tm)} rows → {paths['team']}")
    ...
    if not df_dr.empty:
        ...
        save_dataframe(
            pd.DataFrame([{
                ...
                "score_windows_generated": len(analysis_windows),
            }]),
        ...
        save_dataframe(
            pd.DataFrame([{
                ...
                "score_dirs": args.out_scores_dir,
            }]),
