#!/usr/bin/env python3
"""
new_02b_build_model_input_regression.py
...
A diferencia de la rama de clasificación, aquí los targets son continuos:
    jungle_presence_score
    support_roam_score
    team_side_focus_score

Por defecto guarda el parquet en data_new/training/.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Construye model_input multi-output continuo uniendo draft features con score tables."
    )
    parser.add_argument("--draft-path", default=None,
                        help="Ruta base de draft_features.parquet.")
    parser.add_argument("--jungle-scores-path", default=None,
                        help="Ruta base de jungle_scores.parquet.")
    parser.add_argument("--support-scores-path", default=None,
                        help="Ruta base de support_scores.parquet.")
    parser.add_argument("--team-scores-path", default=None,
                        help="Ruta base de team_tendency_scores.parquet.")
    parser.add_argument("--out-path", default=None,
                        help="Ruta base de salida del model_input parquet.")
    parser.add_argument("--summary-dir", default=None,
                        help="Directorio base para tablas de análisis.")
    parser.add_argument(
        "--join-how",
        choices=["left", "inner"],
        default="left",
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Carga score tables generadas para una ventana concreta (ej. 8 -> *_m08.parquet).",
    )
...
def select_score_columns(score_df: pd.DataFrame, score_col: str, include_run_meta: bool) -> pd.DataFrame:
    keep = JOIN_KEYS + [score_col]
    if include_run_meta:
        keep.extend([c for c in ("max_minute", "window_tag") if c in score_df.columns])
    keep = [c for c in keep if c in score_df.columns]
...
def main() -> None:
    ...
    task_specs = [
        ("jungle", jungle_scores_path, JUNGLE_SCORE_COL),
        ("support", support_scores_path, SUPPORT_SCORE_COL),
        ("team", team_scores_path, TEAM_SCORE_COL),
    ]

    for idx, (task_name, path, score_col) in enumerate(task_specs):
        if not Path(path).exists():
            raise SystemExit(f"No se ha encontrado la tabla de scores para {task_name}: {os.path.abspath(path)}")
        print(f"[Rutas] Cargando {task_name} scores (Entrada): {os.path.abspath(path)}")
        task_df = load_parquet(path)
        validate_required_columns(task_df, f"{task_name}_scores", JOIN_KEYS + [score_col])
        validate_no_duplicate_keys(task_df, f"{task_name}_scores")
        task_df = select_score_columns(task_df, score_col, include_run_meta=(idx == 0))
    ...
    expected_targets = [JUNGLE_SCORE_COL, SUPPORT_SCORE_COL, TEAM_SCORE_COL]
