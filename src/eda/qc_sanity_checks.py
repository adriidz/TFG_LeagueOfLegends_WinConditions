...
DEFAULT_OUTPUT_DIR = os.path.join("data/clean", "qc_reports")
...
def main() -> None:
    ...
    if target_frac is not None and 0.0 < target_frac < 1.0:
        limit = max(1, int(len(match_paths) * target_frac))
        match_paths = match_paths[:limit]
        print(f"Muestreo aplicado ({target_frac}): Limitado a {limit} partidas para QC.")
