...
def derive_player_mvp_labels(row, role_prefix, side_ratio=None, group_ratio=None):
    ...
    early_score = gd15 + 150.0 * td15
    late_proxy = gd20

    delta = early_score - late_proxy
    p_early = sigmoid(delta / 600.0)  # scale to avoid saturation
    ...
    p_side = sigmoid(style_delta / 0.15)  # 0.15 ~= moderate difference
    ...
    row[f"Y_{role_prefix}_STYLE_CLASS"] = "STYLE_SIDE" if p_side >= 0.5 else "STYLE_GROUP"
